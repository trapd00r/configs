...
# Move current window to next workspace on the same monitor
# Alt+Shift+l will move window to next workspace on focused monitor
...
if [ "$current_workspace" -ge "$max_ws" ]; then
    next_workspace=$min_ws
else
