...
#define DEFAULT_SCROLLTHUMB_SIZE  10
...
*/
        ...
        GdkPoint points[3]; 
        points[0].x = x+width-8;  points[0].y = y+height-6;
        points[1].x = x+width-6;  points[1].y = y+height-8;
        points[2].x = x+width-6;  points[2].y = y+height-6;
	    ...
	    const gchar * detail, gint x, gint y, gint width, gint height)
{
  ...
  if (strcmp (detail, "check") == 0) {	/* Menu item */
    ...
    gtk_paint_box (style, window, GTK_STATE_NORMAL, 
		   ...
		   detail, x, y, width + 3, height + 3);
    gdk_draw_rectangle (window, style->base_gc[state_type], TRUE, 
			x + 1,  y + 1, width + 1, height + 1);
    if (shadow_type == GTK_SHADOW_IN) {
      draw_part (window, style->text_gc[state_type], area,
		 x, y, CHECK_TEXT);
      draw_part (window, style->dark_gc[state_type], area,
		 x, y, CHECK_AA);
    }
  }
}
